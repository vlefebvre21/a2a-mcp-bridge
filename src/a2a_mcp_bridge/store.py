...
import sqlite3
import uuid
from datetime import datetime, timedelta, timezone as UTC  # Python 3.9-3.10
...
class Store:
    ...
    *signal_dir* is ``None``, calling :meth:`subscribe` raises
    ...
    def register_capability(
        ...
        Uses INSERT OR REPLACE to handle updates (UNIQUE constraint on agent_id, skill_id).
        """
        self._conn.execute(
            """
    ...
    def get_capabilities(
        ...
        Returns a list of dicts with keys: agent_id, skill_id, domain, description,
        ...
        if keyword:
            kw = f"%{keyword}%"
            conditions.append(
                "(skill_id LIKE ? OR domain LIKE ? OR description LIKE ?)"
            )
